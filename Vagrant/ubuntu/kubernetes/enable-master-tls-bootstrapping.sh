...
cat <<EOF | sudo -u "$(logname)" tee bootstrap-token-07401b.yaml > /dev/null
apiVersion: v1
kind: Secret
metadata:
...
sudo -u "$(logname)" kubectl create -f bootstrap-token-07401b.yaml --kubeconfig admin.kubeconfig
